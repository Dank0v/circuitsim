...
- **ngspice compat modes:** `none`, `hs`, `ps`, `ki`, `lt` — `lt` is LTspice mode: pre-fills the installed LTspice's component databases (`lib\cmp\standard.dio/bjt/mos` — thousands of diode/BJT/VDMOS-MOSFET cards; UTF-16 files are transcoded automatically) and reads vendor `.sub`/`.lib` macromodels, with LTspice `TABLE(...)` B-sources rewritten to ngspice `pwl(...)`. ADI-encrypted models (`<Binary File>`) and OTA-based models (UniversalOpAmp2, newer ADI `.lib`s) cannot run outside LTspice. `ki` is KiCad mode: point the paths box at KiCad's model libraries (pre-filled with the installed KiCad's built-in `Simulation_SPICE.sp`; add its demo/vendor `.lib`/`.mod` files per project) and the runner sets ngspice's PSpice+LTspice translation for included files, the same compatibility KiCad itself uses. Discrete MOSFET/BJT/diode blocks automatically emit the right SPICE device for what the library defines — a `.model` card (e.g. LTspice-style VDMOS power FETs, junction diodes) or a `.subckt` macromodel (e.g. KiCad's BJTs, packaged diodes) — and the amplifier block's pin order matches KiCad's 5-pin op-amp convention (IN+ IN− VCC VEE OUT) directly
...
- **Insert it into a Subcircuit block.** Place the 5×5 **Subcircuit** block, right-click to open it, and drop the chip into the **Chip** slot. The second slot shows the generated `.subckt` netlist (scrollable, read-only, with a **Copy** button); the third slot shows a 3D top-down render of the saved layout (the actual block models, viewed from above) with a zoom slider on its side. The block's top texture changes to reflect the pin count, and a floating name tag appears above it (toggle with the labels key).
